...
class Simulator:
	...
	def init_objects(self, robot, map_file):
		...
		self._screen = pygame.display.set_mode((self._arena_size[0], self._arena_size[1]))
	...
	def add_waypoint(self, pos):
		self._waypoint = pos
		pygame.draw.rect(self._background, (0,0,128), (pos[1] * self.BLOCK_SIZE , pos[0] * self.BLOCK_SIZE, self.BLOCK_SIZE, self.BLOCK_SIZE))
		pygame.draw.rect(self._background, (0,0,0), (pos[1] * self.BLOCK_SIZE , pos[0] * self.BLOCK_SIZE, self.BLOCK_SIZE, self.BLOCK_SIZE), 1)
		self._screen.blit(self._background, (0,0))
		pygame.display.update()
	...
	def run_exploration(self, robot, map_file):
		...
		while True:
			...
			for event in pygame.event.get():
				...
				# 		_explore = Exploration(self._explore_map, self._real_map, self._robot, 300, 3600, (self._screen, self._background))
				# 		_explore.run()
				# 	elif 450 + self._wp_rect.left < _mouse[0] < 450 + self._wp_rect.right and self._wp_rect.top < _mouse[1] < self._wp_rect.bottom:
				# 		_row = self.ask(self._screen, "Enter Waypoint (row)")
				# 		_col = self.ask(self._screen, "Enter Waypoint (col)")

				# 		self._screen.blit(self._background, (0,0))
				# 		self._screen.blit(self._menu, (ArenaConstant.ARENA_COL.value * self.BLOCK_SIZE,0))

				# 		self._explore_map.set_waypoint(((int(_row), int(_col))))
				# 		self._explore_map.draw((self._screen, self._background))
				# 		self._waypoint = (int(_row), int(_col))
				...
				# 			_go_to_wp.do_fastest_path(self._waypoint)
